...
def filtereduserlist(request):
    ...
    for char in beginswith:
        up = char.upper()
        low = char.lower()
        users.extend(User.objects.filter(name__startswith=up).order_by('name').filter(active=True))
        users.extend(User.objects.filter(name__startswith=low).order_by('name').filter(active=True))
